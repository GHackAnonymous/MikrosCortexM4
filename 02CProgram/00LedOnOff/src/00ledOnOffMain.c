...
int main(void)
{
  ...
  pGPIOF_ODR1=(uint32_t*)(0x40000000U+0x00020000U+0x1400U+0x14); //0x40021414U
	...
	pBoton=(uint32_t*)(0x40000000U+0x00020000U+0x1400U+0x10);
   ...
   for(;;)
  {
    *pGPIOF_ODR1 ^= (1<<6); //Led 1 on
		...
		*pGPIOF_ODR1 ^= (0<<9); //Led 4 off
		if(*pBoton & (0x01<<13)){
			for(i=0;i<0200000;i++);  //denbora pasatzeko bukle bat
		}else{
...
void initGPIO(void)
{
  ...
  *(pGPIOF+0U) |= 1 << (9*2);
	...
	//bouton
	pBoton=(uint32_t*)(0x40000000U+0x00020000U+0x1400U+0x0U);
  *(pBoton+0U) &= ~ (3<< (13*2)); 
  *(pBoton+0U) |= 1 << (13*2);
